...
注意：键盘调起时，ascii_mode 通常为 false（即中文模式），用户切换到英文模式时，ascii_mode 会变为 true。所以此选项的最佳用法是将中文模式的定义放在 params 中，将英文模式的定义放在 params.whenAsciiModeOn 中。如果把过来，把英文模式的定义放在 params 中，把中文模式的定义放在 params.whenAsciiModeOff 中，则会出现气泡显示的文本与按键显示不符的问题。
...
```
上面的配置表示，当 ascii_mode 关闭时（即中文模式），显示为大写 B；params 中的 action 定义则自动被推导为在 ascii_mode 打开时生效（即英文模式），显示为小写 b。（不过注意，此写法即使在中文输入状态下，气泡显示的文本仍然是小写 b）
